...
* Add `dcc_report_model()` as a validated single source for three audience
  renderers: bilingual redacted staff Excel/HTML/text, complete statistical
  tables and methods with SHA-256 manifests, and deterministic versioned
  machine JSON/JSONL with bundled schemas.
* `dcc_run()` can atomically publish selected `staff/`, `statistical/`, and
  `machine/` bundles. Run manifests expose report lifecycle states, common
  counts and hashes, and retain cleaning evidence on renderer failure. PDF is
  optional and is not generated by the base contract.
* Add Phase E acceptance and release engineering: generated property/fault
  suites, immutable multi-format fixtures with three-platform CI, reviewed
