...
5. 校验与修正分离：执行阶段只处理违规清单中的条目，不做临时探测
6. 自动修正遵循最小修改原则（Fellegi–Holt），能推断修正的不插补，能插补的不删除
7. 修正动作本身也是声明式规则（若…则改…），与探测规则同库管理
8. 原始数据只读；执行输出新版本数据集，版本间可 diff

**阶段三：可审计报告（Report）** —— 每一格的变化都可追溯到规则和证据

9. 逐条修改留痕：记录 ID、变量、原值 → 新值、触发规则、修正方法、执行时间、执行者/版本
10. 报告双层输出：管理层摘要（质量维度得分、修改量统计）+ 审计明细（cell 级修改日志），可复核、可回滚
11. 审计日志本身是机器可读数据（而非仅 PDF），支持"从成品数据任一单元格反查其清洗历史"
12. 整条 探测→执行→报告 流水线一键复现：同样的原始数据 + 同样的规则版本 = 同样的成品与报告
...
5. 码本驱动的批量重命名/重编码/打标签（对标 iecodebook，同为差异化机会）
6. 标签与特殊缺失码处理（对标 labelled/naniar，调查数据差异化重点）
7. 声明式修正规则 + 推断式修正（对标 dcmodify/deducorrect，最小修改）
8. 插补接口（对标 simputation，统一接口、可插拔）
9. 去标识化辅助（PII 扫描与脱敏）

**可审计报告**

10. cell 级修改日志（机器可读）+ 反查接口：任一单元格 → 其清洗历史
11. 双层质量报告：管理层摘要（质量维度得分）+ 审计明细（对标 dataReporter/pointblank 报告 + DIME 留痕要求）
12. 流水线复现命令：原始数据 + 规则版本 → 一键重建成品与报告
